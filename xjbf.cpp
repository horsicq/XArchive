...
// stream.  They are identical in every known archive and are what U3's
// detector (FUN_00623950) keys on, so they are kept as a cheap first filter in
...
bool XJBF::parseContext(CONTEXT *pContext, PDSTRUCT *pPdStruct)
{
    ...
    QPointer<XJBF> guardedThis(this);
    ...
    if (!guardedThis || !guardedSource || (baLead.size() != 8)) return false;
    ...
    if (!guardedThis || !guardedSource || (baCount.size() != 1)) return false;
    ...
    for (qint32 i = 0; i < nCount; ++i) {
        ...
        // U3 rejects the archive when any of the three 32-bit fields is
