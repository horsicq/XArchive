...
namespace {
// ---------------------------------------------------------------------------
// Layout, recovered from the Micro Focus COBOL Library handler of U3.unp.exe
// (class zgb, VMT 0x0066a7c8; detector 0x0066a850, worker 0x0066a8a0) and then
// confirmed byte-exact against U3's own extraction over the whole 50-file
// corpus.
//
...
//   +0x94  u32le  == 1   (checked by the U3 detector)
//   +0x98  u32le  == 1   (checked by the U3 detector)
//
...
//   +0x0c  u16be  DOS time  (U3 hands this one to the TIME argument)
//   +0x0e  u16be  DOS date
//   +0x10  u16    flags; only 0x0000 and 0x0001 occur
//   +0x12  u8     name length, then that many name bytes (no terminator)
//
// The payload is stored, never compressed: U3 seeks to (block << 7) and copies
// the byte count straight out (FUN_0041d690 -> FUN_00416580, a plain stream
// copy, with no decoder in the path).
// ---------------------------------------------------------------------------
...
bool XLbrCobol::parseContext(CONTEXT *pContext, PDSTRUCT *pPdStruct)
{
    ...
    // Exactly the U3 detector's five tests: the two halves of the magic, the
    ...
    for (qint32 i = 0; i < nCount; i++) {
        ...
        // U3 passes the +0x0e field as the DATE argument and the +0x0c field
        // as the TIME argument (FUN_0066a8a0 -> FUN_00414610); the order is the
