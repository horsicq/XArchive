...
QString XSLS::memberName()
{
    ...
    // U3 names the single member after the archive file itself, extension
    // included; there is nothing else in the container to name it with.
    const QString sPath = XBinary::getDeviceFileName(guardedSource.data());
    QString sResult;
    if (!sPath.isEmpty()) {
...
bool XSLS::parseContext(CONTEXT *pContext, PDSTRUCT *pPdStruct)
{
    ...
    QPointer<XSLS> guardedThis(this);
    QPointer<QIODevice> guardedSource(getDevice());
    ...
    // The size word is read SIGNED, exactly as U3 does; negative is a reject.
