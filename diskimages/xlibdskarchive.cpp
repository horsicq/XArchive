                                ...
                                PDSTRUCT *pPdStruct)
{
...

    {
        ...
        result = XLegacyDiskDecoder::decode(source, maxOutput, &decodeResult,
                                             &decodeError);
        if (!result || decodeResult.images.isEmpty()) {
            if (!decodeError.isEmpty())
                setPdStructErrorString(pPdStruct, decodeError);
            result = false;
            goto failed;
        }
        for (qint32 imageIndex = 0; imageIndex < decodeResult.images.size();
             ++imageIndex) {
            const XLegacyDiskDecoder::IMAGE &image =
                decodeResult.images.at(imageIndex);
            if (image.rawImage.isEmpty() || image.rawImage.size() > maxOutput) {
                result = false;
                goto failed;
            }
            MEMBER member;
            member.rawImage = image.rawImage;
            member.sName = image.name;
            member.nRawSize = image.rawImage.size();
        ...
        for (qint32 memberIndex = 0;
            ...
            // A driver that names its own images (QRST v5 calls them
            // Image1.img / Image2.img) keeps that name; a single-image driver
            // leaves it empty and the archive's own name is used.
            if (context->listMembers[memberIndex].sName.isEmpty())
                context->listMembers[memberIndex].sName =
                    base + QStringLiteral(".img");
        }
        ...
        pState->nNumberOfRecords = context->listMembers.size();
                                                   ...
                                                   PDSTRUCT *pPdStruct)
{
    ...
    if (!context || pState->nNumberOfRecords != context->listMembers.size() ||
        pState->nCurrentIndex >= context->listMembers.size())
        return ARCHIVERECORD();
    const MEMBER &member = context->listMembers.at(pState->nCurrentIndex);
    if (member.rawImage.size() != member.nRawSize || member.nRawSize <= 0)
        return ARCHIVERECORD();
    ...
    record.mapProperties.insert(FPART_PROP_UNCOMPRESSEDSIZE, member.nRawSize);
    record.mapProperties.insert(FPART_PROP_REPORTEDMETHOD,
                                context->sDriver.toUpper());
    if (!markArchiveStreamRecord(&record, pState->nCurrentIndex))
        return ARCHIVERECORD();
    return record;
}
                                   ...
                                   PDSTRUCT *pPdStruct)
{
    ...
    if (pState->spOutputBudget) {
        if (!pState->spOutputBudget->beginEntry(pState->nCurrentIndex,
            ...
            return false;
        if (!pState->spOutputBudget->debit(member.nRawSize) &&
            pState->spOutputBudget->isEnforcing())
            return false;
    }
...
{
    UNPACK_OPERATION_GUARD operationGuard(&m_bUnpackOperationInProgress);
    if (!operationGuard.isAcquired() || !pState || pState->nCurrentIndex < 0 ||
        pState->nCurrentIndex >= pState->nNumberOfRecords ||
        !isUnpackSourceCurrent(pState, pPdStruct))
        return false;
