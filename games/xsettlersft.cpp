...
// The reference implementation refuses a member larger than this, and so does this reader.
                               ...
                               PDSTRUCT *pPdStruct)
{
    ...
    // The reference implementation's identifying predicate, spelled out: the first member starts right
    ...
    for (qint32 i = 0; i < context.nEntryCount; ++i) {
        ...
        // The palette is looked up the way the reference implementation does it - the first entry whose
    ...
    // members could not be produced, and the reference implementation gives up as well.
    if ((nPaletteOffset < 0) ||
        (nPaletteOffset >
