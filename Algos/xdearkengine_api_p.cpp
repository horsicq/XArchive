...
// XDeark private engine API implementation. XDearkEngine owns these calls;
// individual decoder modules use the lower-level utility API instead.
...
void de_destroy(deark *c)
{
	...
	if(c->mp_data) {
		for(i=0; i<c->mp_data->count; i++) {
			...
			// The .fn field is owned by the caller.
		}
