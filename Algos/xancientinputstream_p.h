...
#ifndef XANCIENTINPUTSTREAM_P_H
#define XANCIENTINPUTSTREAM_P_H
...
public:
	ForwardInputStream(const ByteBuffer &buffer,size_t startOffset,size_t endOffset,size_t overrunAllowance=0);
	...
	uint16_t readLE16();
	uint32_t readLE32();
	std::shared_ptr<const ByteBuffer> consume(size_t bytes);
...
public:
	BackwardInputStream(const ByteBuffer &buffer,size_t startOffset,size_t endOffset);
...
template<typename T>
struct InputReadByteWord
{
	T &inputStream;

	std::pair<uint32_t,uint8_t> operator()()
	{
		return std::make_pair(uint32_t(inputStream.readByte()),uint8_t(8));
	}
};

template<typename T>
struct InputReadBE16Word
{
	T &inputStream;

	std::pair<uint32_t,uint8_t> operator()()
	{
		return std::make_pair(uint32_t(inputStream.readBE16()),uint8_t(16));
	}
};

template<typename T>
struct InputReadBE32Word
{
	T &inputStream;

	std::pair<uint32_t,uint8_t> operator()()
	{
		return std::make_pair(inputStream.readBE32(),uint8_t(32));
	}
};

template<typename T>
struct InputReadLE16Word
{
	T &inputStream;

	std::pair<uint32_t,uint8_t> operator()()
	{
		return std::make_pair(uint32_t(inputStream.readLE16()),uint8_t(16));
	}
};
...
public:
	...
	{
		return readBitsGeneric(count,InputReadByteWord<T>{_inputStream});
	}

	uint32_t readBitsBE16(uint32_t count)
	{
		return readBitsGeneric(count,InputReadBE16Word<T>{_inputStream});
	}

	uint32_t readBitsBE32(uint32_t count)
	{
		return readBitsGeneric(count,InputReadBE32Word<T>{_inputStream});
	}

	uint32_t readBitsLE16(uint32_t count)
	{
		return readBitsGeneric(count,InputReadLE16Word<T>{_inputStream});
	}
