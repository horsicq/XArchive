...
#ifdef DEARK_H_INC
#error "xdearkengine_types_p.h included multiple times"
#endif
#define DEARK_H_INC

#ifndef DEARK_CONFIG_H_INC
// Implementation files that don't need any non-default system headers don't have to
// explicitly #include xdearkengine_config_p.h.
...
#include "xdearkengine_config2_p.h"
#else
// In-line default xdearkengine_config2_p.h:
...
// A typed conversion object preserves the old allocation call sites without
// relying on C's implicit void-pointer conversions. Allocation remains owned
// by the codec context and is released with de_free().
class XDearkMemory final
{
public:
    XDearkMemory(void *data = nullptr) : m_data(data) {}
...
XDearkMemory de_malloc(deark *c, i64 n);
XDearkMemory de_mallocarray(deark *c, i64 nmemb, size_t membsize);
XDearkMemory de_realloc(deark *c, void *m, i64 oldsize, i64 newsize);
XDearkMemory de_reallocarray(deark *c, void *m, i64 oldnmemb,
                             size_t membsize, i64 newnmemb);
