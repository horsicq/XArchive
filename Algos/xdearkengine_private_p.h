...
#error "xdearkengine_private_p.h included multiple times"
#endif
#define DEARK_PRIVATE_H_INC

#ifndef DEARK_H_INC
#include "xdearkengine_types_p.h"
#endif
...
const char *de_strchr(const char *s, int c);
#define de_strlen   strlen
#define de_strcmp   strcmp
#define de_strncmp  strncmp
#define de_memcmp   memcmp
template <typename T>
inline T *xdeark_memcpy(T *destination, const void *source, size_t size)
{
    return static_cast<T *>(memcpy(destination, source, size));
}

template <typename T>
inline T *xdeark_memmove(T *destination, const void *source, size_t size)
{
    return static_cast<T *>(memmove(destination, source, size));
}

template <typename T>
inline T *xdeark_memset(T *destination, int value, size_t size)
{
    return static_cast<T *>(memset(destination, value, size));
}

template <typename T>
inline T *xdeark_memchr(T *data, int value, size_t size)
{
    return static_cast<T *>(memchr(data, value, size));
}

#define de_memcpy   xdeark_memcpy
#define de_memmove  xdeark_memmove
#define de_memset   xdeark_memset
#define de_zeromem(a,b) memset((a),0,(b))
#define de_memchr   xdeark_memchr
#ifdef DE_WINDOWS
...
// automatically.
// [For other output functions, see de_puts, de_printf (xdearkengine_types_p.h).]
