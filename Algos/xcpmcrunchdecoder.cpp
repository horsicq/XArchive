...
class Output {
public:
    Output(std::vector<std::uint8_t> *data, std::size_t limit, XCpmCrunchDecoder::Cancel canceled, void *context, bool measureOnly)
    ...
    // RLE90 count includes the already emitted preceding byte.
...
private:
    ...
    std::size_t m_limit;
    XCpmCrunchDecoder::Cancel m_canceled;
    void *m_context;
    bool m_measureOnly;
    std::size_t m_size = 0;
...
// The reference implementation. The original global dictionary is operation-local.
class Crunch {
public:
...
private:
    ...
    bool position(unsigned *distance)
    {
        ...
        // Canonical expansion equals the reference implementation bytes [256] and [16].
...
struct CompactBitReader {
    ...
    unsigned operator()() const
    {
        ...
        // The reference implementation has literal at root bit 1, escape at 00, EOF at 01.
        return bit ^ 1;
    }
    Bits *m_bits;
};

// with strict final-byte padding from recognizer.
// The existing Ancient primitive represents the reference implementation's ordered-frequency tree
...
bool XCpmCrunchDecoder::parseHeader(const std::uint8_t *data, std::size_t size, Header *header)
{
    ...
    *header = Header();
    ...
    // at most 64 filename bytes, optional 15-byte BBS stamp,
...
bool XCpmCrunchDecoder::decode(const std::uint8_t *data, std::size_t size, std::size_t outputLimit,
