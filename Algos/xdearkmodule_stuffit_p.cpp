...
// -------- LZ+Huffman (type 13) decompression --------

static void do_decompr_lzhuff13(deark *c, struct de_dfilter_in_params *dcmpri,
	struct de_dfilter_out_params *dcmpro, struct de_dfilter_results *dres)
{
	u8 *cmpr = NULL;
	u8 *unc = NULL;
	...
	if(!dcmpro->len_known || dcmpri->len < 0 || dcmpro->expected_len < 0 ||
		dcmpri->len > DE_MAX_SANE_OBJECT_SIZE ||
		dcmpro->expected_len > DE_MAX_SANE_OBJECT_SIZE) {
		de_dfilter_set_errorf(c, dres, "stuffit13", "Invalid stream size");
		goto done;
	}
	if(dcmpro->expected_len == 0) goto done;
	if(dcmpri->len == 0) {
		de_dfilter_set_errorf(c, dres, "stuffit13", "Unexpected end of file");
		goto done;
	}
	cmpr = de_malloc(c, dcmpri->len);
	unc = de_malloc(c, dcmpro->expected_len);
	dbuf_read(dcmpri->f, cmpr, dcmpri->pos, dcmpri->len);
	rc = xstuffit13_decode(cmpr, (size_t)dcmpri->len, unc,
		(size_t)dcmpro->expected_len);
	if(rc != 0) {
		de_dfilter_set_errorf(c, dres, "stuffit13",
			"Invalid or truncated LZ+Huffman stream (code %d)", rc);
		goto done;
	}
	dbuf_write(dcmpro->f, unc, dcmpro->expected_len);
...
static const struct cmpr_meth_info cmpr_meth_info_arr[] = {
	...
	{ CMPR_MW, "MW", NULL },
	{ CMPR_LZHUFF, "LZ+Huffman", do_decompr_lzhuff13 },
	{ 14, "installer", NULL },
	{ 15, "Arsenic", NULL }
};
