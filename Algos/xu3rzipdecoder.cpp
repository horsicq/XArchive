// U3 005ef920/005ef4c0/005ef5d0/005ef6e0; see provenance for exact address roles.
#include "xu3rzipdecoder.h"
...
public:
    ...
    bool open()
    {
        ...
        // chunk. U3 005ef920 advances the offset base for every such header.
        qint32 empty = 0;
...
private:
    ...
    bool load(int stream)
    {
        ...
        if ((type != 3 && type != 4) || packed <= 0 || raw <= 0 || packed > XU3RzipDecoder::MaxBlock || raw > m_memoryLimit ||
...
bool XU3RzipDecoder::parseHeader(const QByteArray &data, HEADER *header)
{
...
bool XU3RzipDecoder::decode(const QByteArray &data, XBinary::DATAPROCESS_STATE *writer, RESULT *result, XBinary::PDSTRUCT *progress)
{
    HEADER header;
