...
#include "xlha_legacy_bits_p.inc"
#include "xlha_legacy_pma_p.inc"
...
typedef struct {
	...
	XLhaLegacyDecoderCallback callback;
	void *callback_data;
	size_t eof_padding_remaining;
} LHAPM1Decoder;
...
static size_t read_callback_wrapper(void *buf, size_t buf_len, void *user_data)
{
	...
	if (result == 0) {
        if (buf_len > decoder->eof_padding_remaining) return 0;
        decoder->eof_padding_remaining -= buf_len;
...
static int lha_pm1_init(void *data, XLhaLegacyDecoderCallback callback,
                        void *callback_data)
{
	...
	decoder->eof_padding_remaining = 16;
...
const XLhaLegacyDecoderType xlha_legacy_pm1_decoder = {
	lha_pm1_init,
	NULL,
	lha_pm1_read,
