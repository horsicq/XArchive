...
bool FreezeDecoder::detectHeader(uint32_t hdr,uint32_t footer) noexcept
{
	return ((hdr>>16)==0x1f9eU||(hdr>>16)==0x1f9fU);
}

std::shared_ptr<CodecDecoder> FreezeDecoder::create(const ByteBuffer &packedData,bool exactSizeKnown,bool verify)
{
	return std::make_shared<FreezeDecoder>(packedData,exactSizeKnown,verify);
}

FreezeDecoder::FreezeDecoder(const ByteBuffer &packedData,bool exactSizeKnown,bool verify) :
...
const std::string &FreezeDecoder::getName() const noexcept
{
...
size_t FreezeDecoder::getPackedSize() const noexcept
{
...
size_t FreezeDecoder::getRawSize() const noexcept
{
	// same thing, decompression needed first
	return _rawSize;
}

void FreezeDecoder::decompressImpl(ByteBuffer &rawData,bool verify)
{
	...
	FreezeBitReader readBit{bitReader};
	...
	for(;;)
	{
		...
		if (code<256U)
		{
			...
			distance|=bitReader.readBits8(distanceBits);
			distance++;
