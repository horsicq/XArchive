...
const qint32 MAX_COLUMNS = 1024;  // U3 00678fe0.
...
// U3 00678680: the ninth varint byte contributes eight bits.
...
// Table pages and overflow chains follow U3 00678700, 006788b0,
...
public:
    ...
    bool header()
    {
        ...
        if (pageSize == 1) pageSize = 65536;  // Current SQLite representation; U3's zero sentinel was erroneous.
...
private:
              ...
              const std::function<bool(const QList<Field> &)> &callback)
    {
        ...
        if (type != 5 && type != 13) return fail("SQLite WITHOUT ROWID/index B-tree is not supported by U3 text export");
...
XBinary::ARCHIVERECORD XSQLiteArchive::infoCurrent(UNPACK_STATE *pState, PDSTRUCT *pPdStruct)
{
    ...
    result.mapProperties.insert(FPART_PROP_REPORTEDMETHOD, QStringLiteral("U3 SQLite table text (UTF-16LE)"));
