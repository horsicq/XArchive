...
bool XRecognita::parseContext(CONTEXT *pContext, PDSTRUCT *pPdStruct)
{
    ...
    while (nOffset < context.nInputSize) {
        ...
        if (!guardedSource || (baHeader.size() != RECOGNITA_HEADER_SIZE + 2)) return false;
        ...
        // These are the DETECTION rules and belong to the first record only.
        // Applying them to every record rejected three archives over a single
        // member with a two-character extension.
        const bool bFirstRecord = context.listMembers.isEmpty();
        if (bFirstRecord) {
        ...
        if (bFirstRecord && ((static_cast<quint8>(pHeader[25]) != RECOGNITA_DCL_LITERAL) || (static_cast<quint8>(pHeader[26]) != RECOGNITA_DCL_DICT))) {
            return false;
        }
