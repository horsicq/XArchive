...
public:
    ...
    bool readString(QString *psResult)
    {
        ...
        if (nTag == TC_STRING) {
            ...
            // U3 reads a 32-bit length here; a real TC_LONGSTRING is 64-bit,
            // but IzPack never emits one and matching U3 keeps the byte
            // positions identical.
            quint32 nLength = 0;
    ...
    // buffered the next few primitive writes when it flushed.  U3 accepts an
...
bool XIzPack::parseContext(CONTEXT *pContext, PDSTRUCT *pPdStruct)
{
    ...
    for (qint32 nIndex = 0; nIndex < context.nDeclaredCount; nIndex++) {
        ...
        if (bPack200Jar) {
            // A Pack200-compressed JAR: the payload is not the file, so U3
