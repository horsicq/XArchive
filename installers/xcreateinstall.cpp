                               ...
                               QList<XMaterializedUnpackGuard *> *pCompanionGuards, XBinary::PDSTRUCT *pPdStruct)
{
    ...
    for (int i = 1; i < h.nVolumeCount; i++) {
        ...
        if (!ciReadOpenFileExact(guardedVolumeFile, 10, &baVolumeHeader, pPdStruct) || !guardedVolumeDevice || !guardedVolumeFile) return false;
...
bool XCreateInstall::initUnpack(UNPACK_STATE *pState, const QMap<UNPACK_PROP, QVariant> &mapProperties, PDSTRUCT *pPdStruct)
{
    ...
    if (!m_bTrustedSnapshot) {
        ...
        if (!isProgressAlive() || !pSnapshot || (getDeviceGeneration() != nGeneration) || (getDevice() != guardedSource))
            return false;
        const QString sSourceFileName = ciDeviceFileName(guardedSource);
    ...
    const INTERNAL_INFO *pInfo = static_cast<const INTERNAL_INFO *>(getInternalInfo(pPdStruct));
    if (!isProgressAlive() || !pInfo || !pInfo->bIsValid) return false;
    ...
    if (info.nGeaOffset >= 0) {
        ...
        if (!isProgressAlive() || (baHeader.size() != 73) || !XBinary::isPdStructNotCanceled(pPdStruct)) {
            delete pContext;
            return false;
        }
        quint64 nDeclaredEnd = ciRd64(reinterpret_cast<const quint8 *>(baHeader.constData()) + 42);
        const qint64 nCurrentSize = getSize();
        if (!isProgressAlive() || (nCurrentSize < 0) || (nDeclaredEnd < (quint64)info.nGeaOffset) ||
        ...
        if (!isProgressAlive() || (baArchive.size() != nArchiveSize) || !XBinary::isPdStructNotCanceled(pPdStruct)) {
            delete pContext;
            return false;
        }
        ...
        if (!isProgressAlive() || sResolvedFileName.isEmpty()) {
            delete pContext;
            return false;
        }
        ...
        if (!isProgressAlive() || !pExternalPrimaryGuard) {
            delete pContext;
            return false;
        }
        QIODevice *guardedExternalDevice = pExternalPrimaryGuard ? pExternalPrimaryGuard->device() : nullptr;
        QFile *pExternalPrimaryFile = guardedExternalDevice ? dynamic_cast<QFile *>(guardedExternalDevice) : nullptr;
        QFile *guardedExternalFile = pExternalPrimaryFile;
        qint64 nArchiveSize = guardedExternalFile ? guardedExternalFile->size() : -1;
        if (!isProgressAlive() || !guardedExternalDevice || !guardedExternalFile || (nArchiveSize < 73) ||
            (nArchiveSize > CI_MAX_ARCHIVE_SIZE) || (nArchiveSize > INT_MAX)) {
            delete pContext;
            return false;
        }
        const bool bArchiveRead = ciReadOpenFileExact(guardedExternalFile, nArchiveSize, &baArchive, pPdStruct);
        if (!isProgressAlive() || !bArchiveRead || !guardedExternalDevice || !guardedExternalFile) {
            delete pContext;
            return false;
        }
        const qint64 nArchiveSizeAfter = guardedExternalFile->size();
        if (!isProgressAlive() || !guardedExternalDevice || !guardedExternalFile || (nArchiveSizeAfter != nArchiveSize)) {
            delete pContext;
            return false;
        }
        const bool bAtEnd = guardedExternalFile->atEnd();
        if (!isProgressAlive() || !guardedExternalDevice || !guardedExternalFile || !bAtEnd ||
    ...
    if (header.nFlags & 0x0002u) {
        ...
        if (!isProgressAlive() || ((quint32)baInfo.size() != header.nInfoSize)) {
            delete pContext;
            return false;
        }
    } else {
    ...
    if (!isProgressAlive() || !bLogicalDataBuilt) {
        delete pContext;
        return false;
    }
    ...
    for (int nFileIndex = 0; nFileIndex < listFiles.size(); nFileIndex++) {
        ...
        while ((quint64)baFileData.size() < file.nSize) {
            ...
            if (nMethod == 0) {
                ...
                if (!isProgressAlive() || (baDecoded.size() != nSolidOffset + nOutputSize)) {
                    bArchiveOk = false;
                    break;
                }
                ...
                if (!isProgressAlive() || (baBlock.size() != nOutputSize)) {
                    bArchiveOk = false;
                    break;
                }
                baLzgeHistory.clear();
    ...
    if (!isProgressAlive() || (nDataPos != baLogicalData.size())) bArchiveOk = false;
    ...
    if (!isProgressAlive() || !bCompanionsFinal || !bSourceFinal || !bCompanionsCurrent || (getDeviceGeneration() != nGeneration) ||
        (getDevice() != guardedSource) || !isPdStructNotCanceled(pPdStruct)) {
        delete pContext;
        return false;
    }
...
bool XCreateInstall::unpackCurrent(UNPACK_STATE *pState, QIODevice *pDevice, PDSTRUCT *pPdStruct)
{
    ...
    const AUTHENTICATION_PROBE isAuthenticated = {this, pLifetimeState, pContext, pState, nIndex};
