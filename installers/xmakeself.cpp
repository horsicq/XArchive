...
// few kilobytes; the reference implementation scans the same 64 KiB window for the payload magic.
...
bool XMakeself::parseContext(CONTEXT *pContext, PDSTRUCT *pPdStruct)
{
    ...
    if (nPayloadOffset < 0) {
        // The reference implementation's fallback: scan the first 64 KiB for the payload's
