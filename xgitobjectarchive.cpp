...
// U3 007346b0: decimal length terminated by NUL, at most twenty digits.
...
// U3 00424220 / 00423900, applied to the original basename without removing its extension.
...
bool XGitObjectArchive::probe(QIODevice *device, qint64 *size, PDSTRUCT *pd)
{
...
bool XGitObjectArchive::initUnpack(UNPACK_STATE *state, const QMap<UNPACK_PROP, QVariant> &properties, PDSTRUCT *pd)
{
    ...
    if (!context || !probe(getDevice(), &context->size, pd) || !self) {
...
bool XGitObjectArchive::unpackCurrent(UNPACK_STATE *state, QIODevice *device, PDSTRUCT *pd)
{
    ...
    // U3 00734750 checks the nineteen hash bytes represented by a loose object's basename.
