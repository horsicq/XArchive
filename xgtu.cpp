...
{
    // U3's FUN_00547410: the key starts at the stored name length and is then
                         ...
                         PDSTRUCT *pPdStruct)
{
    ...
    // Phase 1: frames whose plaintext belongs to a previous volume.  U3 walks
...
bool XGTU::parseContext(CONTEXT *pContext, PDSTRUCT *pPdStruct)
{
    ...
    while (!bTerminated && isPdStructNotCanceled(pPdStruct)) {
        ...
        // U3 reads +0x04 as a single u32 and demands exactly 1, which makes it
