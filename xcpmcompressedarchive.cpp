...
bool XCpmCompressedArchive::readHeader(XU3CpmDecoder::Header *pHeader, PDSTRUCT *pPdStruct, qint64 *pUncompressedSize, qint64 nOutputLimit)
{
    QPointer<XCpmCompressedArchive> owner(this);
    const qint64 nSize = getSize();
    if (!pHeader || !isPdStructNotCanceled(pPdStruct) || nSize < 4 || nSize > static_cast<qint64>(XU3CpmDecoder::MaxInput)) return false;
    const QByteArray data = read_array_process(0, qMin<qint64>(nSize, 160), pPdStruct);
    if (!owner || !XU3CpmDecoder::parseHeader(reinterpret_cast<const std::uint8_t *>(data.constData()), static_cast<std::size_t>(data.size()), pHeader)) return false;
    const bool bType = (m_fileType == FT_CPM_CRUNCH && pHeader->format == XU3CpmDecoder::Format::Crunch) ||
                       (m_fileType == FT_CPM_LZH && pHeader->format == XU3CpmDecoder::Format::Crlzh) ||
                       (m_fileType == FT_UNIX_COMPACT && pHeader->format == XU3CpmDecoder::Format::UnixCompact);
    ...
    if (pUncompressedSize || pHeader->format == XU3CpmDecoder::Format::UnixCompact) {
        const QByteArray packed = read_array_process(0, nSize, pPdStruct);
        if (!owner || packed.size() != nSize) return false;
        XU3CpmDecoder::Result measured;
        const bool valid = XU3CpmDecoder::decode(reinterpret_cast<const std::uint8_t *>(packed.constData()), static_cast<std::size_t>(packed.size()),
...
bool XCpmCompressedArchive::initUnpack(UNPACK_STATE *pState, const QMap<UNPACK_PROP, QVariant> &mapProperties, PDSTRUCT *pPdStruct)
{
    ...
    *pState = UNPACK_STATE();
    ...
    qint64 nLimit = static_cast<qint64>(XU3CpmDecoder::MaxOutput);
    ...
    if (context->header.format == XU3CpmDecoder::Format::UnixCompact) {
...
XBinary::ARCHIVERECORD XCpmCompressedArchive::infoCurrent(UNPACK_STATE *pState, PDSTRUCT *pPdStruct)
{
    ...
    const XU3CpmDecoder::Header &header = static_cast<CONTEXT *>(pState->pContext)->header;
...
bool XCpmCompressedArchive::unpackCurrent(UNPACK_STATE *pState, QIODevice *pDevice, PDSTRUCT *pPdStruct)
{
    ...
    if (nInputSize < 4 || nInputSize > static_cast<qint64>(XU3CpmDecoder::MaxInput)) return false;
    OUTPUT_POLICY policy = {};
    ...
    XU3CpmDecoder::Result decoded;
    if (!XU3CpmDecoder::decode(reinterpret_cast<const std::uint8_t *>(input.constData()), static_cast<std::size_t>(input.size()),
