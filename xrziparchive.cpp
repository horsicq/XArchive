// U3 RZIP wrapper; the private decoder stages and checks complete two-stream chunks.
...
bool XRzipArchive::readHeader(XU3RzipDecoder::HEADER *header, PDSTRUCT *progress)
{
    QPointer<XRzipArchive> owner(this);
    ...
    if (!owner || !source || size < 24 || size > XU3RzipDecoder::MaxInput) return false;
    const QByteArray bytes = read_array_process(0, 24, progress);
    if (!owner || !source || !XU3RzipDecoder::parseHeader(bytes, header)) return false;
...
bool XRzipArchive::unpackCurrent(UNPACK_STATE *state, QIODevice *device, PDSTRUCT *progress)
{
    ...
    if (inputSize < 24 || inputSize > XU3RzipDecoder::MaxInput) return false;
    ...
    XU3RzipDecoder::HEADER header;
    if (!XU3RzipDecoder::parseHeader(input, &header) || header.rawSize != context.header.rawSize || header.major != context.header.major || header.minor != context.header.minor)
        return false;
    ...
    XU3RzipDecoder::RESULT decoded;
    if (!XU3RzipDecoder::decode(input, &writer, &decoded, progress) || !owner || !source || !output ||
