...
struct XLegacyStoreArchive::EntryBuilder
{
    ...
    bool operator()(qint64 headerOffset, qint64 headerSize,
                    ...
                    const QByteArray &compressProperties = QByteArray())
    {
        ...
        entry.baCompressProperties = compressProperties;
                                     ...
                                     PDSTRUCT *pPdStruct)
{
    ...
    if (ft == FT_SSBOB) {
        ...
        if (std::memcmp(p, "FPAK", 4) == 0) {
            ...
            // NOT a version - it is the number of members in the whole
            // distribution set, so a large set carries a large value here.
            if (version < 1 || declaredPacked < 1 ||
        ...
        // A volume whose middle is damaged still holds every segment ahead of
        // the damage.  Everything the walk cannot make sense of therefore ends
        // the chain instead of rejecting the file, but only behind at least one
        // whole segment, so a lone bogus header is still refused.  The reader
        // that opens the file (XFpakArchive::readVolume) carries the identical
        // rule: these two validators are independent and BOTH decide FT_FPAK,
        // so a file either passes both or the format is detected and then
        // refuses to open.
        bool truncated = false;
        while (pos < total) {
            bool malformed = !rangeWithin(total, pos, 30) ||
                             std::memcmp(p + pos, "FPPF", 4) != 0;
            // +4 is the PKZIP general-purpose flag word and +6 the method, and
            // both are live: the reference family carries flags 0, 8 and 14,
            // and method 0 (stored) on 48 segments. Only 0x02 (8 KiB window)
            // and 0x04 (literal tree) change the bit stream.
            ...
            qint32 nameSize = 0;
            if (!malformed) {
                segmentFlags = qFromLittleEndian<quint16>(p + pos + 4);
                segmentMethod = qFromLittleEndian<quint16>(p + pos + 6);
                dosTime = qFromLittleEndian<quint16>(p + pos + 8);
                dosDate = qFromLittleEndian<quint16>(p + pos + 10);
                crc32 = qFromLittleEndian<quint32>(p + pos + 12);
                packedSize = qFromLittleEndian<quint32>(p + pos + 16);
                rawSize = qFromLittleEndian<quint32>(p + pos + 20);
                segmentSize = qFromLittleEndian<quint32>(p + pos + 24);
                nameSize = qFromLittleEndian<quint16>(p + pos + 28);
                malformed = ((segmentMethod != 0) && (segmentMethod != 6)) ||
                            packedSize < 1 || rawSize < 1 || segmentSize < 1 ||
                            packedSize > MAX_LEGACY_STORE_SIZE ||
                            rawSize > MAX_LEGACY_STORE_SIZE ||
                            segmentSize > packedSize || nameSize < 1 ||
                            nameSize > 512 ||
            ...
            QString name;
            if (!malformed) {
                name = fixedName(p + pos + 30, nameSize);
            ...
            // The method and the flag word BOTH have to travel with the
            // record.  XDecompress falls back to method 6 / flags 0 when the
            // property is absent, which is a real Implode profile: a stored
            // member would be run through the Implode decoder, and a member
            // that sets 0x02 (8 KiB dictionary) or 0x04 (literal tree) would
            // be decoded against the wrong bit stream.  The four bytes below
            // are the {method, flags} pair decFpakProfile() reads back.
            QByteArray fpakProfile;
            if (complete) {
                fpakProfile.resize(4);
                qToLittleEndian<quint16>(
                    segmentMethod, reinterpret_cast<uchar *>(fpakProfile.data()));
                qToLittleEndian<quint16>(
                    segmentFlags,
                    reinterpret_cast<uchar *>(fpakProfile.data()) + 2);
            }
            if (!addEntry(pos, dataOffset - pos, dataOffset, segmentSize,
                          ...
                          name, complete, crc32, fpakProfile))
                return false;
        ...
        // The lead header's totals describe the COMPLETE distribution set. An
        // intact volume must account for them exactly; a volume the walk itself
        // reported as damaged cannot, and its shortfall is already explained,
        // so the equality is not evidence against it.
        if ((declaredPacked >= 0) && !havePartial && !truncated &&
            ((completePacked != declaredPacked) ||
             (completeRaw != declaredRaw)))
            return false;
        if (truncated) archiveEnd = pos;
