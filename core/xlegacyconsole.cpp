...
bool XLegacyConsole::commandMatches(const QString &token, const char *alias)
{
...
XLegacyConsole::OPTIONS XLegacyConsole::parse(const QStringList &arguments)
{
    OPTIONS o;
    ...
    if (commandMatches(first,"extract") || commandMatches(first,"e") || commandMatches(first,"x")) o.command = EXTRACT;
        ...
        // unknown first token means an archive pathname. The third
...
bool XLegacyConsole::collect(const QString &directory, const QString &relative, const QString &mask, const QString &excluded,
                        QList<TARGET> *targets, QSet<QString> *visited, int depth, QString *error)
{
    if (depth > 256 || targets->size() >= 262144) { *error = QStringLiteral("Batch traversal limit exceeded"); return false; }
    ...
    QList<ENTRY> entries;
#ifdef Q_OS_WIN
    // The reference implementation uses the same FindFirstFileW mask
    ...
    do {
        ...
        if (name != "." && name != "..") {
            if (entries.size() >= 262144) { FindClose(handle); *error = QStringLiteral("Directory entry limit exceeded"); return false; }
    ...
    for (const ENTRY &entry : entries) {
        ...
        if (entry.directory) {
            ...
            if (targets->size() >= 262144) { *error = QStringLiteral("Batch target limit exceeded"); return false; }
...
bool XLegacyConsole::prepare(const OPTIONS &options, PATHS *paths, QString *error)
{
    ...
    *paths = PATHS();
    ...
    if (mutating) {
        ...
        // Preserve the reference implementation's exact-destination exclusion for sort/testsort.
...
QString XLegacyConsole::batchDestination(const OPTIONS &o, const PATHS &p, const TARGET &t)
{
...
XOptions::CR XLegacyConsole::sortFile(const QString &source, const QString &destination, XBinary::FT type)
{
...
XOptions::CR XLegacyConsole::run(XArchiveConsole &console, const OPTIONS &options, const PATHS &paths)
{
    ...
    properties.insert(XBinary::UNPACK_PROP_LEGACY_RENAME,options.rename);
    ...
    for (const TARGET &target : paths.targets) {
        ...
        if (options.command == TYPE || options.command == SORT || options.command == TESTSORT) {
            ...
            // The reference implementation type/sort omit unrecognized inputs; they are never moved merely
...
bool XLegacyConsole::process(XArchiveConsole &console, const QStringList &arguments, qint32 *result)
{
    if (arguments.size() < 2 || arguments.at(1) != "--legacy") return false;
...
QString XLegacyConsole::help()
{
    return QStringLiteral(
        "XFU legacy command adapter\n"
        "Usage: xfileunpackerc --legacy [command] archive-or-pattern [destination]\n"
        ...
        "  x3             x2 plus '(0)name'..'(999)name' collision renaming\n"
        "  help/h/?       this help\n"
        "Default destination: ./Output. Batch archive directory names retain extensions.\n"
        "Use '*' for recursive traversal; the mask applies to directory names too.\n"
