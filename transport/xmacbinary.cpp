                            ...
                            PDSTRUCT *pPdStruct)
{
    ...
    // Checked below rather than here: a header whose four padded extents add up
    // to exactly the file size is a MacBinary regardless of a bad stored CRC,
    // and the reference corpus contains one such file.
    bool bCRCMatches = true;
    if (nVersion == 0)
    {
    ...
    else if (nVersion == 129)
    {
        if (nMinimumVersion != 129)
            return false;
        bCRCMatches = (nStoredCRC == macBinaryCRC(pHeader, 124));
    }
    ...
    // An exact-fit size chain is the stronger signal of the two, so a bad CRC
    // is only forgiven when the file ends precisely where the header says.
    if (!bCRCMatches && (nArchiveEnd != nTotalSize))
        return false;
