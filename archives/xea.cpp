...
const qint32 EA_ZEROS_OFFSET = 0x21;
const qint32 EA_ZEROS_SIZE = 15;
...
bool eaDecodeName(const QByteArray &baHeader, QString *psName)
{
    ...
    // NOT a bug to leave the tail unchecked: the packer overwrites this
    // fixed-width field without clearing it, so whatever follows the
    // terminating NUL is the tail of the PREVIOUS, longer name.  Demanding
    // zero padding rejected 12 of the 27 reference archives.
...
bool XEA::parseContext(CONTEXT *pContext, PDSTRUCT *pPdStruct)
{
    ...
    while (context.listMembers.size() < EA_MAX_MEMBERS &&
        ...
        // Constant across every member of the reference family, and worth
        // checking because the name field cannot be checked past its NUL - see
        // the note in eaReadName.
        if (pHeader[EA_NAME_OFFSET + EA_NAME_SIZE] != 0) return false;
        for (qint32 i = EA_ZEROS_OFFSET; i < (EA_ZEROS_OFFSET + EA_ZEROS_SIZE); ++i) {
        ...
        // Both size words are read as SIGNED and the reference implementation rejects a negative one;
        ...
        // size fields have to agree, exactly as the reference implementation's stored branch requires.
