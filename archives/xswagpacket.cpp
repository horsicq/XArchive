...
// widths below were measured against the reference implementation's extraction of all 227 corpus files
...
qint64 XSwagPacket::trimmedTextSize(const QByteArray &baRegion)
{
    ...
    // is space padding. The reference implementation drops it; the 0x1A can sit either before or after
                               ...
                               PDSTRUCT *pPdStruct)
{
    ...
    for (qint64 i = 0; i < nCount; i++) {
        ...
        // The reference implementation numbers them, which is the only stable naming this format has.
