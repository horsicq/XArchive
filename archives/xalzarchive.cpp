...
// separator normalization, intermediate dot components,
...
bool XAlzArchive::parse(QIODevice *device, QList<ARCHIVERECORD> *items, const QMap<UNPACK_PROP, QVariant> &properties, PDSTRUCT *pd)
{
    ...
    while (offset < r.size) {
        ...
        // The reference implementation's ANSI conversion uses the Windows system code page; allow the caller's
...
bool XAlzArchive::unpackCurrent(UNPACK_STATE *state, QIODevice *device, PDSTRUCT *pd)
{
    ...
    // The reference implementation supports STORE and raw DEFLATE with CRC-32; method 1 is unsupported.
