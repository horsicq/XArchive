...
// The writer only ever emits 8.3 names, and the reference implementation's own detector rejects a length
...
bool XFiz::parseContext(CONTEXT *pContext, PDSTRUCT *pPdStruct)
{
    ...
    while ((context.listMembers.size() < FIZ_MAX_MEMBERS) && isPdStructNotCanceled(pPdStruct)) {
        ...
        // The reference implementation rejects a member whose size fields have the sign bit set; the same
...
XBinary::HANDLE_METHOD XFiz::methodToHandleMethod(quint8 nMethod)
{
    ...
    if (nMethod == FIZ_METHOD_LH5) {
        // Verified against the reference implementation's decompressor: it drives the shared LHA engine
