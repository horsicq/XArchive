...
// key; the reference implementation keeps it at 0x009f6e58 as a raw 8-byte little-endian quad word.
...
bool XPCSecure::parseContext(CONTEXT *pContext, PDSTRUCT *pPdStruct)
{
    ...
    // The reference implementation requires strictly more than 0x44 bytes: a header with no payload is
    // not a PCSECURE file.
    ...
    if (nVerifier != 0) {
        ...
        // The reference implementation's code and is NOT corpus-verified.
