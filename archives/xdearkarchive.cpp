...
// ISSUE-22. The intermediate ZIP this bridge reads back must carry a
// modification time in every member header, so for a source format that stores
// none the engine is told to write the DOS epoch rather than the current time
// (see Algos/xdearkengine_p.cpp). That makes a listing reproducible, but
// publishing the value would still show 1980-01-01 as though it were the
// member's real date. Absent is the honest answer, and the console already
// renders a missing timestamp as an empty column.
//
// Only the exact DOS epoch is dropped. A member that carried a real timestamp
// keeps it: the engine prefers the source value whenever one exists, so
// reaching this sentinel means the source had nothing to give.
static void dearkDropSynthesizedTimestamp(XBinary::ARCHIVERECORD *pRecord)
{
    if (!pRecord) return;

    // The sentinel arrives in TWO different representations and both have to be
    // recognised, or the console falls back to whichever one survived and the
    // synthesized stamp is still displayed:
    //
    //   FPART_PROP_MTIME comes from the ZIP extended-timestamp ("UT") field and
    //     is a true UTC instant - it compares equal on the epoch value.
    //   FPART_PROP_DATETIME comes from the DOS date/time words. The engine
    //     writes the sentinel's UTC wall clock into them, but a DOS field
    //     carries no zone, so the reader tags it local: its epoch value is off
    //     by this machine's UTC offset while its wall clock reads exactly
    //     1980-01-01 00:00:00.
    //
    // So match either the instant or that wall clock. Matching only one of the
    // two left the stamp visible, and which value appeared depended on the
    // machine's timezone - the kind of bug that looks like nondeterminism.
    const qint64 nDosEpochMSecs = Q_INT64_C(315532800) * 1000;
    const QDate dosEpochDate(1980, 1, 1);
    const QTime dosEpochTime(0, 0, 0);
    const XBinary::FPART_PROP props[2] = {XBinary::FPART_PROP_DATETIME, XBinary::FPART_PROP_MTIME};

    for (qint32 i = 0; i < 2; i++) {
        if (!pRecord->mapProperties.contains(props[i])) continue;
        const QDateTime dateTime = pRecord->mapProperties.value(props[i]).toDateTime();
        if (!dateTime.isValid()) continue;

        const bool bInstantMatches = (dateTime.toMSecsSinceEpoch() == nDosEpochMSecs);
        const bool bWallClockMatches = (dateTime.date() == dosEpochDate) && (dateTime.time() == dosEpochTime);

        if (bInstantMatches || bWallClockMatches) pRecord->mapProperties.remove(props[i]);
    }
}
