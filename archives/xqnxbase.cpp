...
// struct startup_header. The reference implementation only ever probes 0x400 and 0x3f8, but
...
// imagefs_size is range checked, startup_size only has to be
...
bool XQNXBase::readHeader(HEADER *pHeader, PDSTRUCT *pPdStruct)
{
    ...
    if (baPrefix.size() != QNXB_BOOT_PREFIX_SIZE) return false;
    ...
    if ((qint64)baScan.size() != nScanSize) return false;
    ...
    for (qint64 nHeaderOffset = 0; nHeaderOffset <= nLastCandidate; nHeaderOffset += QNXB_HEADER_SCAN_STEP) {
        ...
        // The reference implementation does not look at the signature at all - it identifies
...
bool XQNXBase::parseImage(const QByteArray &baImage, QList<MEMBER> *pListMembers)
{
    ...
    // The reference implementation walks the directory with a "remaining" counter that it
    ...
    // The reference implementation only reports success when it reaches the zero-length
...
bool XQNXBase::parseContext(CONTEXT *pContext, PDSTRUCT *pPdStruct)
{
    ...
    if (!readHeader(&context.header, pPdStruct)) return false;
    ...
    if (baPacked.size() != context.nCompressedSize) return false;

    QByteArray baImage;
    if (!XQNXBaseDecoder::decodeImage(baPacked, context.nImageFsSize, -1, &baImage)) return false;
    // The reference implementation insists the decompressed length is exactly imagefs_size.
