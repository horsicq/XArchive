...
bool XGeniusLibrary::parseContext(CONTEXT *pContext, PDSTRUCT *pPdStruct)
{
    ...
    QPointer<XGeniusLibrary> guardedThis(this);
    ...
    if (!guardedThis || !guardedSource || (baHeader.size() != GPL_HEADER_SIZE)) return false;
    ...
    for (qint32 i = 0; i < nCount; ++i) {
        ...
        if (!guardedThis || !guardedSource || (baEntry.size() != GPL_ENTRY_SIZE)) return false;
