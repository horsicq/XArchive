...
bool XSQZ::parseInternalInfo(INTERNAL_INFO *pInfo, PDSTRUCT *pPdStruct)
{
    ...
    while (XBinary::isPdStructNotCanceled(pPdStruct)) {
        ...
        if (nHeaderLength == 0) {
            ...
            // has none. The header test below is what keeps detection tight.
        ...
        if (nHeaderLength < SQZ_MIN_HEADER_LENGTH) {
            ...
            // The skip is NOT uniform: a type-1 comment block carries a
            // second u16 that REPLACES the first and is followed by five more
            // bytes, and type 4 has two more. Using one rule for all of them
            // lands in the middle of a comment payload.
            qint64 nSkipHeader = 3;
            quint16 nExtraSize = read_uint16(nOffset + 1);
            if (nHeaderLength == 1) {
                if ((nFileSize - nOffset) < 10) return false;
                nExtraSize = read_uint16(nOffset + 3);
                nSkipHeader = 10;
            ...
            nOffset += nSkipHeader + static_cast<qint64>(nExtraSize);
            continue;
        }
