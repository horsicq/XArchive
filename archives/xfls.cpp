                      ...
                      PDSTRUCT *pPdStruct)
{
    ...
    for (qint64 i = 1; i < nRecords; ++i) {
        ...
        // 0x20 or 0x00 and the sixth is a free-form ASCII letter taking at
        // least ten values ('F', 'Y', 'E', 'P', 'D', 'R', 'C', 'O', 'Z',
        // 'M'), so pinning it to a pair rejected 292 of the 485 archives
        // whose file-level structure is identical.
        ...
        if (!rangeWithin(nTotalSize, nRecordOffset, FLS_RECORD_SIZE) ||
            ...
            baMemberTag.left(4) != QByteArray("\x00\x00\x00\x01", 4) ||
            ((static_cast<quint8>(baMemberTag.at(4)) != 0x20U) &&
             (static_cast<quint8>(baMemberTag.at(4)) != 0x00U))) {
            return false;
        }
