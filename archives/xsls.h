...
// only header-level reject the reference implementation performs)
//   +0x0d           LZHUF bit stream to EOF
//
// There is no stored name, no timestamp and no checksum: the container holds
// exactly one member and the reference implementation names it after the archive file itself, which is
